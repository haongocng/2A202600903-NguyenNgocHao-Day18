...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if len(current_parent_text) + len(para) + 2 > parent_size and temp_parent_paras:
            parent_text = "\n\n".join(temp_parent_paras)
            ...
            temp_parent_paras = [para]
            current_parent_text = para
        else:
            temp_parent_paras.append(para)
            current_parent_text += ("\n\n" if current_parent_text else "") + para
            
    if temp_parent_paras:
        parent_text = "\n\n".join(temp_parent_paras)
        pid = f"parent_{len(parents)}"
        parents.append(Chunk(
            text=parent_text,
